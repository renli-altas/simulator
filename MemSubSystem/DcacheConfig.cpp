...
int find_wb_entry(uint32_t addr)
{
    ...
    {
        if (write_buffer[i].valid && write_buffer[i].addr == (addr & ~(DCACHE_LINE_BYTES - 1)))
        {
            return i;
        }
    }
...
void write_dcache_line(uint32_t set_idx, uint32_t way, uint32_t tag, uint32_t data[DCACHE_LINE_WORDS])
{
    ...
    valid_array[set_idx][way] = true;
    dirty_array[set_idx][way] = false;
    tag_array[set_idx][way] = tag;
    lru_reset(set_idx, way);
    for (int w = 0; w < DCACHE_WAYS; w++)
    {
        data_array[set_idx][way][w] = data[w];
    }
}
