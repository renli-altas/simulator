...
void Dcache::comb_hit()
{
    ...
    bool same_st_ld = (cpu_st_in.req && cpu_ld_in.req &&tag_st == tag_ld && index_st == index_ld);
    if (!hit_st&& cpu_st_in.req)
    {
        if (hit_way_st == -1)
    ...
    if (!hit_ld && cpu_ld_in.req && !uncache_access)
    {
        if (hit_way_ld == -1){
            
            hit_way_ld = same_st_ld?hit_way_st:getlru(index_ld);
        }
        miss_deal(index_ld, hit_way_ld, tag_ld, dirty_writeback_ld, paddr_ld, tag_ld_way);
        miss_num++;
    }
    else if(hit_ld && cpu_ld_in.req && !uncache_access)
    {
        hit_num++;
...
void Dcache::seq()
{
    ...
    uint32_t out_index = 59;
    uint32_t out_way = 2;
    uint32_t out_offset = 2;
