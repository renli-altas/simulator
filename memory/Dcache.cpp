...
void Dcache::seq()
{
    ...
    // }
    // if(old_cache_tag_debug != dcache_tag[out_index][out_way]){
    //     old_cache_data_debug = dcache_tag[out_index][out_way];
    //     printf("Debug Dcache seq 3 sim_time:%lld dcache_data[%d][%d][%d]:0x%08x data_tag:0x%08x data_valid:%d data_issued:%d dcache_dirty:%d\n",sim_time,out_index,out_way,out_offset, old_cache_data_debug, dcache_tag[out_index][out_way], dcache_valid[out_index][out_way], dcache_issued[out_index][out_way], dcache_dirty[out_index][out_way]);
    // }
    // if(io.cpu_st_in->req == true && io.cpu_st_in->addr &0xfffffffc ==0x8fe11efc){
