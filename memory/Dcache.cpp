...
void Dcache::comb_in()
{
    ...
    else
    {
        ...
        {
            write_data(io.mem_st, write_data_st,get_addr(tag_st, index_st, offset_st) & 0xfffffffc, offset_id_st);
        }
        else if (state_st == DCACHE_READ)
        {
            read_data(io.mem_st, get_addr(tag_st, index_st, offset_st) & 0xfffffffc, offset_id_st);
        }
        else
        {
            transfer_zero(io.mem_st);
    ...
    else
    {
        ...
        {
            write_data(io.mem_ld, write_data_ld, get_addr(tag_ld, index_ld, offset_ld) & 0xfffffffc, offset_id_ld);
        }
        else if (state_ld == DCACHE_READ)
        {
            read_data(io.mem_ld, get_addr(tag_ld, index_ld, offset_ld) & 0xfffffffc, offset_id_ld);
        }
        else
        {
            transfer_zero(io.mem_ld);
...
{
    if (hit_st == true&&req_st_reg&&!io.control->flush)
    {
    ...
    else
    {
        if (state_ld == DCACHE_READ)
        {
            ...
            if (!flush_flag_ld&&offset_id_ld==offset_ld){
                io.cpu_ld->rdata = io.mem_ld->data.data;
            }
        }
        else
        {
...
//================================================================================
        ...
        if(io.cpu_ld->req==false||io.cpu_ld->data_ok==true||io.control->flush){
            req_ld_reg = false;
        }else{
            req_ld_reg = true;
        }

        if(io.cpu_st->req==false||io.cpu_st->data_ok==true||io.control->flush){
            req_st_reg = false;
        }else{
            req_st_reg = true;
        }

        if (hit_st && io.cpu_st->req == true && state_st == DCACHE_IDLE&&!io.cpu_st->data_ok)
        {
            updatelru(index_st);
...
//================================================================================
    if (state_ld == DCACHE_WRITE)
    {
        ...
        {
            dirty_writeback_ld = false;
            dcache_tag[index_ld][hit_way_ld] = 0;
...
//================================================================================
    if (state_st == DCACHE_WRITE)
    {
        ...
        {
            dirty_writeback_st = false;
            dcache_tag[index_st][hit_way_st] = 0;
